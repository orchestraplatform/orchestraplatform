...
| `resources.ephemeralStorage` | string | no | `"8Gi"` | Ephemeral storage **limit** — the kubelet's eviction threshold for everything written outside the `/data` PVC (package installs, `/tmp`, container writable layer). A cluster defaults this to 1Gi when unset, which Bioconductor sessions exceed. |
| `resources.ephemeralStorageRequest` | string | no | `"1Gi"` | Ephemeral storage **request** — what the scheduler bin-packs on. Measured sessions use ~50MB; setting this to the limit caps a node at `floor(allocatable / limit)` sessions regardless of machine size. |
