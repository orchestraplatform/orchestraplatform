...
<Aside type="tip">
This page is the **controller reference** (middlewares, auth flow, nginx). For the
end-to-end GKE Standard procedure — static IP, Traefik pinned to it, cert-manager
DNS-01 wildcard, and the default `TLSStore` — follow
[4. Ingress, TLS & auth](/docs/deployment/ingress-tls-auth/) in the
...
on `oauth2Proxy.fullProxy` (see [What the chart creates](#what-the-chart-creates)).

<Aside type="tip">
For Traefik, set `oauth2Proxy.fullProxy: true`. In full-proxy mode oauth2-proxy
sits in front of the frontend and issues a clean login redirect; in the default
forwardAuth mode (`fullProxy: false`) Traefik v3 returns a `401` + `Location`
header instead of a transparent `302`. Both `values-prod.yaml` and the GCP
reference deployment use `fullProxy: true`.
</Aside>
