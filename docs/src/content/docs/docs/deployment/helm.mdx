...
frontend, an optional bundled oauth2-proxy, and an optional bundled PostgreSQL
for dev/test.

<Aside type="tip">
This is the **full values reference**. For the ordered install procedure (values
layering, fresh-install ordering, the `systemPool` block, and the operator
`tierMap`), follow [3. Install Orchestra (Helm)](/docs/deployment/install/) in the
[deployment sequence](/docs/deployment/overview/).
</Aside>
...
| PostgreSQL 14+ | External — see [Database](#database) below. For dev/test, the chart can bundle one: [Bundled PostgreSQL](#bundled-postgresql-devtest) |
...
For local, CI, or demo installs the chart can deploy its own Postgres (the
Bitnami `postgresql` subchart) so a single `helm install` is self-contained —
no external database and no hand-created secret:
...
When enabled, `ORCHESTRA_DATABASE_URL` for the API and the migrate job is
derived automatically from `postgresql.auth.*` (default
`orchestra:orchestra@<release>-postgresql:5432/orchestra`), overriding
`api.database.*`. The migrate job runs as a `post-install` hook on fresh
installs (instead of `pre-install`) and waits for Postgres to accept
connections before migrating.

<Aside type="caution">
Dev/test only. The credentials are plain chart values and the data lives on a
single in-cluster PVC. Production keeps `postgresql.enabled=false` and uses
`api.database.existingSecret`.
</Aside>

Because Bitnami moved versioned images to a paid catalog in 2025, the chart
pins the freely pullable `bitnamilegacy/postgresql` image matching the
subchart's appVersion. Any other key under `postgresql.*` passes through to the
[Bitnami subchart](https://github.com/bitnami/charts/tree/main/bitnami/postgresql).
...
Bundled dev/test Postgres — see [Bundled PostgreSQL](#bundled-postgresql-devtest).

| Key | Default | Description |
|---|---|---|
| `postgresql.enabled` | `false` | Deploy the bundled Postgres and auto-wire the database URL (dev/test only) |
| `postgresql.auth.username` | `orchestra` | |
| `postgresql.auth.password` | `orchestra` | Plain chart value — dev/test only |
| `postgresql.auth.database` | `orchestra` | |
| `postgresql.image.repository` | `bitnamilegacy/postgresql` | Free mirror of the Bitnami image (versioned Bitnami images went paid in 2025) |
| `postgresql.*` | | Everything else passes through to the [Bitnami subchart](https://github.com/bitnami/charts/tree/main/bitnami/postgresql) |
