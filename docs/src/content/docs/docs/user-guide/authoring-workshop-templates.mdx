...
description: The field reference the submission form is built on — every field of a workshop template, and the fallback for hand-authoring the YAML directly.
---
...
<Aside type="tip" title="Most people never write this YAML">
  The front door is the [submission form](/docs/contribute/submit/): fill in a
  structured issue and a bot renders the YAML and opens the PR after a maintainer
  approves. This page is the **field reference the form is built on** — the schema
  behind the form, and the contract for the fallback when you hand-author a file.
</Aside>
...
in-memory registry at startup — there is no admin UI and no runtime database
toggle. A catalog change is a change to these files, whether the
[submission form](/docs/contribute/submit/) writes it for you (the front door)
or you hand-author the file and open the PR yourself (the fallback).
...
| `submittedBy`     | string \| null       | no       | `null`                  | GitHub handle of the submitter, **stamped by the front door from the issue author** ([ADR-0009](/docs/adr/0009-template-front-door/)); **absent on hand-authored templates** — you don't set this. |

<Aside type="note" title="Where tier + resources come from via the form">
  The submission form has no `tier`/`resources` fields — it offers a single
  **Size** dropdown (Small / Standard / Large / X-Large). The front door expands
  that preset into `tier` and `resources` when it renders the YAML, so `size` is a
  **front-door concept that never appears in a template file**. Hand-authors set
  `tier` and `resources` directly (below).
</Aside>
...
<Aside type="tip">
  **Prefer the [issue form](/docs/contribute/submit/).** It renders this YAML for
  you and opens the PR after a maintainer approves. Hand-author only when you want
  full control over the file — the steps below are the fallback, not the primary
  path.
</Aside>
